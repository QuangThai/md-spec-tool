      ...
      <div className="flex w-full gap-2">
        {transcriptSplits.map((split) => {
          ...
          return (
            <span
              key={split.id}
              className="flex h-8 items-center justify-center truncate rounded-lg bg-linear-to-r from-rose-300 to-pink-400 px-2 text-[10px] font-medium text-white"
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
        <section className="min-w-0 space-y-5">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/2 px-6 py-5">
            ...
            <div className="mt-4">
              {previewSupported ? (
                <div className="max-w-full overflow-x-auto" style={{
                  scrollbarWidth: 'thin',
                  scrollbarColor: 'rgba(242, 123, 47, 0.3) rgba(0, 0, 0, 0.1)'
                }}>
                  <div
                    ref={waveformRef}
                    className="min-w-full rounded-xl bg-black/40 p-3"
                  />
                </div>
              ) : (
              ...
              <div className="mt-3">{captionRail}</div>
            </div>
        ...
        <aside className="min-w-0 self-start rounded-2xl border border-white/10 bg-white/2 px-6 py-5 lg:sticky lg:top-6">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="rounded-full border text-white border-accent-orange/50 bg-accent-orange px-4 py-1.5 text-xs font-medium transition hover:border-accent-orange/70 hover:bg-accent-orange/30"
            >
              {loading ? "Processing..." : `Transcribe${response ? "" : " (1)"}`}
            </button>
          </div>
