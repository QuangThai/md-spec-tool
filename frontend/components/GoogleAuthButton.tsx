"use client";

import { useGoogleAuth } from "@/hooks/useGoogleAuth";
import { LogOut, Loader2 } from "lucide-react";

export function GoogleAuthButton() {
  const { connected, loading, error, email, login, logout } = useGoogleAuth();

  if (connected) {
    return (
      <div className="flex items-center gap-3 px-4 py-2 bg-white/5 rounded-lg text-sm border border-white/10 hover:border-white/20 transition">
        <div className="flex flex-col min-w-0">
          <span className="text-white/70 text-xs">Connected</span>
          <span className="text-white font-medium truncate">{email || "Google Account"}</span>
        </div>
        <button
          onClick={logout}
          disabled={loading}
          className="ml-2 p-1.5 hover:bg-white/10 rounded transition disabled:opacity-50 flex-shrink-0"
          title="Disconnect Google Account"
          aria-label="Disconnect Google Account"
        >
          <LogOut size={16} />
        </button>
      </div>
    );
  }

  if (error) {
    return (
      <div className="flex items-center gap-2 text-xs">
        <span className="text-red-400">{error}</span>
        <button
          onClick={login}
          disabled={loading}
          className="text-blue-400 hover:underline disabled:opacity-50 transition"
          aria-label="Retry Google authentication"
        >
          Retry
        </button>
      </div>
    );
  }

  return (
    <button
      onClick={login}
      disabled={loading}
      className="flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-lg hover:bg-white/10 hover:border-white/20 transition disabled:opacity-50 text-sm text-white font-medium"
      aria-label={loading ? "Connecting to Google Account..." : "Connect Google Account"}
    >
      {loading ? (
        <>
          <Loader2 size={16} className="animate-spin" />
          <span>Connecting...</span>
        </>
      ) : (
        <>
          <svg
            className="w-4 h-4"
            viewBox="0 0 24 24"
            fill="currentColor"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
            <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
            <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
            <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
          </svg>
          <span>Connect Google</span>
        </>
      )}
    </button>
  );
}
