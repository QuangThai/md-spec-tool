...
export function SharePageContent() {
  ...
  const { shareData, error } = useMemo(() => {
    if (!encoded) {
      return {
        shareData: null as ShareData | null,
        error: "No share data found in URL" as string | null,
      };
    }
    ...
    if (!decoded) {
      return {
        shareData: null as ShareData | null,
        error: "Invalid or corrupted share link" as string | null,
      };
    }
    return { shareData: decoded, error: null as string | null };
  }, [encoded]);
